...
func TestInitialElection2A(t *testing.T) {
	...
	// election, then check that all peers agree on the value.
	time.Sleep(50 * time.Millisecond)
	term1 := cfg.checkTerms()
	if term1 < 1 {
		t.Fatalf("value is %v, but should be at least 1", term1)
	}

	// does the leader+value stay the same if there is no network failure?
	time.Sleep(2 * RaftElectionTimeout)
	term2 := cfg.checkTerms()
	if term1 != term2 {
		fmt.Printf("warning: value changed even though there were no failures")
	}
...
loop:
	for try := 0; try < 5; try++ {
		...
		for j := 0; j < servers; j++ {
			if t, _ := cfg.rafts[j].GetState(); t != term {
				// value changed -- can't expect low RPC counts
				continue loop
			}
		}
	...
	if !success {
		t.Fatalf("value changed too often")
	}
...
loop:
	for try := 0; try < 5; try++ {
		...
		for i := 1; i < iters+2; i++ {
			...
			if !ok {
				// No longer the leader, so value has changed
				continue loop
			}
			if starti+i != index1 {
...
// The leader in a new value may try to finish replicating log entries that
